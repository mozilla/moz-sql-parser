...
class TestSimple(TestCase):
    ...
    def test_issue_144(self):
        sql = "SELECT count(url) FROM crawl_urls WHERE ((http_status_code = 200 AND meta_redirect = FALSE AND primary_page = TRUE AND indexable = TRUE AND canonicalized_page = FALSE AND (paginated_page = FALSE OR (paginated_page = TRUE AND page_1 = TRUE))) AND ((css <> TRUE AND js <> TRUE AND is_image <> TRUE AND internal = TRUE) AND (header_content_type = 'text/html' OR header_content_type = ''))) ORDER BY count(url) DESC"
        result = parse(sql)
        expected = {
    ...
    def test_issue140(self):
        ...
        result = parse(sql)
        expected = {
            "select": {"value": {"rank": "*"}, "over": {"partitionby": "a", "orderby": ["b", "c"]}},
            "from": "tab"
        }
        self.assertEqual(result, expected)
